...
app.controller('ProductDetailController', ['$scope','$rootScope','localStorageService', '$location', 'authService', function ($scope,$rootScope,localStorageService, $location, authService) {
    ...
    function init() {
        $scope.GetProductDetail($scope.Tempproduct[4]);
        $scope.GetRelatedPrdoucts($scope.Tempproduct[4]);
        $scope.GetReviews($scope.Tempproduct[4]);
        $scope.Similarproduct($scope.Tempproduct[4]);
    }
    ...
    $scope.GetProductDetail = function (ID) {
        ...
        var _model = { ProductId: itemNew[4] }
        $.ajax({
            ...
            success: function (data, textStatus, xhr) {
                ...
                $scope.Image = $scope.product._AllProductImage;
                CheckScopeBeforeApply();
                applyslider();
    ...
    $('.dec-btn').on("click", function () {
        debugger;
        $scope.quantities = parseInt($('input.quantity-no').val()) - 1;
        if ($scope.quantities == 0) {
            $scope.quantities = 1;
        }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                console.log($scope.similarproduct);
                    $scope.$apply();
                    $(".owl-carousel.owl-theme.similarproduct").owlCarousel({

                        nav: true, // Show next and prev buttons

                        dots: false,
                        items: $scope.similarproduct.length,
                        margin: 15,
